...
import { Fonts, RegistryKeys } from '../types/keys';
...
export function showNicknamePrompt(scene: Phaser.Scene, opts: NicknameOpts = {}): void {
  ...
  const root = scene.add.container(0, 0).setDepth(120);
  const dim = scene.add.rectangle(0, 0, GAME_WIDTH, GAME_HEIGHT, 0x05060a, 0.86).setOrigin(0).setInteractive();
  const card = scene.add.rectangle(cx, cy, GAME_WIDTH - 60, 240, 0x1c1730, 0.99).setStrokeStyle(3, 0xffd23f);
  ...
  const okBtn = scene.add.text(cx + 60, cy + 70, 'OK', {
    fontFamily: Fonts.Display, fontSize: '22px', color: '#ffffff', stroke: '#1a1c2c', strokeThickness: 4,
